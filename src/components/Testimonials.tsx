...
const Testimonials = () => {
  return <section className="py-20 bg-slate-900" id="testimonials">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {testimonials.map((testimonial, index) => <div key={index} className="bg-slate-800 rounded-xl p-8 border border-slate-700 hover:border-blue-500/50 transition-all duration-300">
              <div className="flex items-center mb-6">
                <Quote className="w-8 h-8 text-blue-400 mr-4" />
                <div className="flex">
                  {[...Array(testimonial.rating)].map((_, starIndex) => <Star key={starIndex} className="w-5 h-5 text-yellow-400 fill-current" />)}
                </div>
              </div>
